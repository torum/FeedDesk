...
internal sealed partial class FeedClientService : BaseClient, IFeedClientService
{
    ...
    public FeedClientService()
    {
        ...
        Client.Timeout = TimeSpan.FromSeconds(15); // 15 sec timeout here
    ...
    public async override Task<HttpClientEntryItemCollectionResultWrapper> GetEntries(Uri entriesUrl, string feedId, CancellationToken token)
    {
        ...
        catch (TaskCanceledException e) when (token.IsCancellationRequested)// when (timoutCts.IsCancellationRequested) // when (ex.InnerException is TimeoutException)
        {
            ...
            HttpReqException(res.Error, e.Message + " (TaskCanceledException IsCancellationRequested)", "Client.GetAsync", "FeedHttpClient:GetEntries");
            res.IsError = true;
        }
    ...
    private static void FillEntryItemFromXmlAtom03(FeedEntryItem entItem, XmlNode entryNode, XmlNamespaceManager atomNsMgr, Uri baseUri)
    {
