...
public class FeedLink(Uri feedUri, FeedLink.FeedKinds feedKind, string title, Uri? siteUri, string siteTitle)
{
    ...
    public FeedKinds FeedKind { get; set; } = feedKind;
}
...
public class SearviceDocumentLinkErr(string errorTitle, string errorDescription) : SearviceDocumentLinkBase
{
...
public class RsdLink : SearviceDocumentLinkBase
{
    ...
    public List<RsdApi> Apis { get; set; } = [];
...
public class ServiceResultErr(string et, string ed) : ServiceResultBase
{
    public string ErrTitle { get; set; } = et;
    public string ErrDescription { get; set; } = ed;
}

// AuthRequired Class (BasedOn ServiceResultBase)
public class ServiceResultAuthRequired(Uri addr) : ServiceResultBase
{
    public Uri Addr { get; set; } = addr;
}
...
public abstract class ServiceResult(ServiceTypes serviceType, Uri endpointUri, AuthTypes authType) : ServiceResultBase
{
    public ServiceTypes ServiceType { get; set; } = serviceType;

    public Uri EndpointUri = endpointUri;

    public AuthTypes AuthType { get; set; } = authType;
}

// AtomPub Service Result Class That Holds NodeService (SearviceDocumentLink info). (BasedOn ServiceResult)
public class ServiceResultAtomPub(Uri endpointUri, AuthTypes authType, NodeService nodeService) : ServiceResult(ServiceTypes.AtomPub, endpointUri, authType)
{
    public NodeService AtomService { get; set; } = nodeService;
}

public class ServiceResultAtomAPI(Uri endpointUri, AuthTypes authType) : ServiceResult(ServiceTypes.AtomApi, endpointUri, authType)
{
}
...
public class AutoDiscoveryService : IAutoDiscoveryService
{
                ...
                if (elements != null)
                {
                    foreach (var e in elements)
                    {
                        ...
                        if (!string.IsNullOrEmpty(re))
                        {
                            ...
                            if (string.Equals(re, "EDITURI", StringComparison.OrdinalIgnoreCase))
                            {
                                if (isFeed)
                                {
                                    continue;
                            ...
                            if (string.Equals(re, "SERVICE", StringComparison.OrdinalIgnoreCase))
                            {
                                if (isFeed)
                                    continue;
                            ...
                            if (string.Equals(re, "https://api.w.org/", StringComparison.OrdinalIgnoreCase))
                            {
                                if (isFeed)
                                    continue;
                            ...
                            if (string.Equals(re, "ALTERNATE", StringComparison.OrdinalIgnoreCase))
                            {
                                if (!isFeed)
                                    continue;
    ...
    private async Task<ServiceResultBase> ParseXml(HttpContent content, Uri addr)
    {
            ...
            if (document != null)
            {
                if (document.DocumentElement != null)
                {
                    ...
                    if (document.DocumentElement.LocalName.Equals("rss"))
                    {
                        ...
                        if (isOK)
                        {
                            ServiceResultFeed rss = new()
                            {
                                FeedlinkInfo = new(addr, FeedLink.FeedKinds.Rss, feedTitle, siteUri, "")
                            };
                    ...
                    else if (document.DocumentElement.LocalName.Equals("feed"))
                    {
                        ...
                        if (isOK)
                        {
                            ServiceResultFeed atom = new()
                            {
                                FeedlinkInfo = new(addr, FeedLink.FeedKinds.Atom, feedTitle, siteUri, "")
                            };
                    ...
                    else if (document.DocumentElement.LocalName.Equals("rsd"))
                    {
                        ...
                        var resRsd = new ServiceResultRsd
                        {
                            Rsd = await ParseRsdAsync(content)
                        };
        ...
        ServiceResultErr ret = new("XML parse error.", "Could not parse the document.");
    ...
    private async Task<ServiceResultBase> ParseAtomServiceDocument(HttpContent content, Uri addr, string userName, string apiKey, AuthTypes authType)
    {
        ...
        catch (Exception e)
        {
            ...
            ServiceResultErr xe = new("XML parse error.", "Could not parse the Atom Service document: " + e.Message);
                ...
                foreach (XmlNode ws in workspaceList)
                {
                    ...
                    if (collectionList != null)
                    {
                        foreach (XmlNode col in collectionList)
                        {
                            ...
                            if (colHrefUri is null)
                            {
                                continue;
                            }
                            NodeAtomPubEntryCollection collection = new("Collection Name", colHrefUri, colHrefUri.AbsoluteUri)
                            {
                                IsExpanded = true,
                                Parent = workspace
                            };
    ...
    private async Task<RsdLink> ParseRsdAsync(HttpContent content)
    {
        ...
        try
        {
            ...
            if (xdoc.Root != null)
            {
                ...
                //var att = xdoc.DocumentElement.Attributes.GetNamedItem("xmlns");
                //var val = att?.Value;

                //if (xdoc.Root.Name.ToString().ToLower() == @"{http://archipelago.phrasewise.com/rsd}rsd") // < this
                if (string.Equals(xdoc.Root.Name.ToString(), @"{http://archipelago.phrasewise.com/rsd}rsd", StringComparison.OrdinalIgnoreCase))
                {
                    //var serv = xdoc.DocumentElement.SelectSingleNode("//rsd/service");
                    var serv = xdoc.Root.Element(aw + "service");
                    if (serv != null)
                    {
                        //rsdDoc.EngineName = serv.SelectSingleNode("engineName")?.Value;
                        //var homePageLink = serv.SelectSingleNode("homePageLink")?.Value;
                        ...
                        //var apis = serv.SelectSingleNode("apis");
                        var apis = serv.Element(aw + "apis");
                        if (apis != null)
                        {
                            //var apiList = apis.SelectNodes("api");
                            var apiList = apis.Elements(aw + "api");
                            if (apiList != null)
                            {
                                foreach (var api in apiList)
                                {
                                    //var apiName =  (api as XmlNode)?.Attributes?.GetNamedItem("name");
                                    //var apiBlogId = (api as XmlNode)?.Attributes?.GetNamedItem("blogID");
                                    //var apiPreferred = (api as XmlNode)?.Attributes?.GetNamedItem("preferred");
                                    //var apiLink = (api as XmlNode)?.Attributes?.GetNamedItem("apiLink");
                    ...
                    {
                        UpdateStatus("Load XML/RSD failed. service element is missing.");
                    }
                }
                else
                {
                    UpdateStatus("Load XML/RSD failed. Document root name or namespace is wrong.");
                }
            }
            else
            {
    ...
    */
    ...
    private static string GenDigest(string password, string created, string nonce)
    {
        ...
        digest = SHA1.HashData(digestBytes);
